...
Pak'nSave shares the Foodstuffs NZ platform with New World — the
API shape and page structure are nearly identical, only the domain differs.
...
CATEGORY_URLS = [
    ...
    "https://www.paknsave.co.nz/shop/category/meat-poultry-and-seafood",
    "https://www.paknsave.co.nz/shop/category/fridge-deli-and-eggs",
    "https://www.paknsave.co.nz/shop/category/bakery",
    "https://www.paknsave.co.nz/shop/category/frozen",
    "https://www.paknsave.co.nz/shop/category/pantry",
    "https://www.paknsave.co.nz/shop/category/hot-and-cold-drinks",
    "https://www.paknsave.co.nz/shop/category/snacks-treats-and-easy-meals",
    "https://www.paknsave.co.nz/shop/category/health-and-body",
    "https://www.paknsave.co.nz/shop/category/household-and-cleaning",
]
...
class PakNSaveScraper(BaseScraper):
    ...
    chain_name = CHAIN_CONFIG["paknsave"]["name"]
    branch_name = CHAIN_CONFIG["paknsave"]["branch_name"]
    ...
    async def scrape(self) -> list[ScrapedProduct]:
        ...
        if self._search_terms:
            urls = [f"https://www.paknsave.co.nz/shop/search?q={t}" for t in self._search_terms]
        ...
        for url in urls:
            ...
            try:
                ...
                # Override store to the configured Pak'nSave store — Playwright has no
                # session so Foodstuffs defaults to a smaller store. Re-fetch page 1.
                ...
                for entry in captured:
                    if "paginated/products" in entry.get("url", ""):
                        ...
                        logger.info(f"  [pagination] totalPages={d.get('totalPages')} totalHits={d.get('totalHits')} url_captured={'yes' if self._api_paginated_url else 'NO'} headers_captured={'yes' if self._api_headers else 'NO'}")
                        break
                else:
    ...
    async def _enrich_with_barcodes(self, products: list[ScrapedProduct]) -> None:
        ...
        # Group ScrapedProducts by productId
        ...
        for pid, items in by_pid.items():
            url = f"https://api-prod.paknsave.co.nz/v1/edge/store/{store_api_id}/product/{pid}"
            try:
